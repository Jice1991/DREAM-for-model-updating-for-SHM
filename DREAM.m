function [chain,output,fx,log_L] = DREAM(Func_name,DREAMPar,Par_info,Meas_info);
% ----------------------------------------------------------------------------------------------%
%                                                                                               %
...
if ~isfield(DREAMPar,'restart') || strcmp(DREAMPar.restart,'no'),
    ...
    [chain,output,X,fx,CR,pCR,lCR,delta_tot,log_L] = DREAM_initialize(DREAMPar,Par_info,Meas_info,f_handle,chain,output,log_L);
...
% Initialize waitbar
h = waitbar(0,'Running DREAM - Please wait...');  totaccept = 0; tic;
...
for t = T_start : DREAMPar.T,
    ...
    [fx_new] = Evaluate_model(xnew,DREAMPar,Meas_info,f_handle);
